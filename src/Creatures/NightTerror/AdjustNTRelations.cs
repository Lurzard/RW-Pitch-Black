...
public class ScareEverything {
    ...
    private static bool Condition(Creature crit) {
        return crit != null && crit.Template.type == Enums.CreatureTemplateType.NightTerror;
    }
    ...
    unsafe private static CreatureTemplate.Relationship AdjustRelationship(Orig* orig, ArtificialIntelligence self, RelationshipTracker.DynamicRelationship dRelation) {
        ...
        if (trackedCreature != null && trackedCreature.Template.type == Enums.CreatureTemplateType.NightTerror) {
    ...
    public static void Apply()
    {
        ...
        unsafe {
            ...
            On.CentipedeAI.IUseARelationshipTracker_UpdateDynamicRelationship += (orig, self, dRelation) => {
                Creature trackedCreature = dRelation?.trackerRep?.representedCreature?.realizedCreature;
                if (trackedCreature != null && trackedCreature.Template.type == Enums.CreatureTemplateType.NightTerror && self.centipede.Template.type != Enums.CreatureTemplateType.NightTerror) {
                    // If the 'target' creature is a NightTerror and self is not, be afraid
                    return new CreatureTemplate.Relationship(newRelation, 10f);
                }
                if (trackedCreature != null && trackedCreature.Template.type == CreatureTemplate.Type.Slugcat && self.centipede.Template.type == Enums.CreatureTemplateType.NightTerror) {
                ...
                else if (trackedCreature != null && trackedCreature.Template.type == Enums.CreatureTemplateType.NightTerror && self.centipede.Template.type == Enums.CreatureTemplateType.NightTerror) {
