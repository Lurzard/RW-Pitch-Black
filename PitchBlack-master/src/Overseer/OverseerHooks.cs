...
public class OverseerHooks
{
    ...
    private static void Player_Update(On.Player.orig_Update orig, Player self, bool eu) {
        ...
        if (self.room?.world?.overseersWorldAI?.playerGuide != null && self.room.game.session is StoryGameSession session && MiscUtils.IsBeaconOrPhoto(session.saveStateNumber) && self.slugcatStats.name == Beacon) {
    ...
    public static void IL_Overseer_Update(ILContext il) {
        try {
            ...
            cursor.EmitDelegate((float val, Overseer self) => { if (self.room.game.session is StoryGameSession session && (session.saveStateNumber == Beacon || session.saveStateNumber == Beacon)) { return -94f; } return val; });
            ...
            cursor.Emit(OpCodes.Ldarg_0);
            cursor.EmitDelegate((float val, Overseer self) => { if (self.room.game.session is StoryGameSession session && (session.saveStateNumber == Beacon || session.saveStateNumber == Beacon)) { return 1700f;} return val;  });
            ...
            cursor.Emit(OpCodes.Ldarg_0);
            cursor.EmitDelegate((float val, Overseer self) => { if (self.room.game.session is StoryGameSession session && (session.saveStateNumber == Beacon || session.saveStateNumber == Beacon)) { return -0.2f;} return val;  });
            ...
            cursor.Emit(OpCodes.Ldarg_0);
            cursor.EmitDelegate((float val, Overseer self) => { if (self.room.game.session is StoryGameSession session && (session.saveStateNumber == Beacon || session.saveStateNumber == Beacon)) { return 0.4f; } return val; });
