...
public class OverseerEx
{
    ...
    public void Update()
    {
        ...
        if (overseer.room.game.session is not StoryGameSession || (overseer.room.game.session is StoryGameSession gameSession && gameSession.saveStateNumber != Plugin.BeaconName && gameSession.saveStateNumber != PhotoName)) { Debug.Log($"Pitch Black {nameof(Update)}: Not a correct StoryGameSession"); return; }
