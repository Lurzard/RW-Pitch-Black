...
public class OverseerEx
{
    ...
    public void Update()
    {
        ...
        if (overseer.room.game.session is not StoryGameSession || (overseer.room.game.session is StoryGameSession gameSession && gameSession.saveStateNumber != Beacon && gameSession.saveStateNumber != Photomaniac)) { Debug.Log($"Pitch Black {nameof(Update)}: Not a correct StoryGameSession"); return; }
