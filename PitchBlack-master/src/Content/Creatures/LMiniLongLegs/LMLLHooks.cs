...
static class LMLLHooks
{
    internal static void Apply()
    {
        ...
        On.MoreSlugcats.StowawayBugAI.WantToEat += (On.MoreSlugcats.StowawayBugAI.orig_WantToEat orig, StowawayBugAI self, CreatureTemplate.Type input) => input != PBEnums.CreatureTemplateType.LMiniLongLegs && orig(self, input);
    ...
    static void DaddyGraphics_DrawSprites(On.DaddyGraphics.orig_DrawSprites orig, DaddyGraphics self, RoomCamera.SpriteLeaser sLeaser, RoomCamera rCam, float timeStacker, Vector2 camPos)
    {
        ...
        if (self.daddy.Template.type == PBEnums.CreatureTemplateType.LMiniLongLegs) {
    ...
    static bool AdjustSizeClass(Func<DaddyLongLegs, bool> orig, DaddyLongLegs self) {
        return self.Template.type != PBEnums.CreatureTemplateType.LMiniLongLegs && orig(self);
    }
    static CreatureTemplate.Relationship DaddyAI_IUseARelationshipTracker_UpdateDynamicRelationship(On.DaddyAI.orig_IUseARelationshipTracker_UpdateDynamicRelationship orig, DaddyAI self, RelationshipTracker.DynamicRelationship dRelation)
    {
        ...
        if (self.creature?.creatureTemplate.type is CreatureTemplate.Type tp && tp != CreatureTemplate.Type.BrotherLongLegs && tp != PBEnums.CreatureTemplateType.LMiniLongLegs && dRelation.trackerRep?.representedCreature is AbstractCreature c && c.creatureTemplate.type == PBEnums.CreatureTemplateType.LMiniLongLegs)
        {
        ...
        if (self.creature?.realizedCreature is LittleLongLegs lmll && dRelation.trackerRep?.representedCreature?.realizedCreature is Creature crit && crit.Template.type != PBEnums.CreatureTemplateType.LMiniLongLegs && lmll.mainBodyChunk.rad > crit.mainBodyChunk.rad*1.1f) {
    ...
    static void On_DaddyLongLegs_ctor(On.DaddyLongLegs.orig_ctor orig, DaddyLongLegs self, AbstractCreature abstractCreature, World world)
    {
        ...
        if (abstractCreature.creatureTemplate.type == PBEnums.CreatureTemplateType.LMiniLongLegs)
        {
    ...
    static void IL_DaddyLongLegs_ctor(ILContext il)
    {
        ...
        if (c.TryGotoNext(x => x.MatchNewarr<BodyChunk>()))
        {
            ...
            c.EmitDelegate((int length, DaddyLongLegs self) => {
                if (self.Template.type == PBEnums.CreatureTemplateType.LMiniLongLegs) {
        ...
        {
            c.Emit(Ldarg_0);
            c.EmitDelegate((int length, DaddyLongLegs self) => self.Template.type == PBEnums.CreatureTemplateType.LMiniLongLegs ? Random.Range(3, 5) : length);
        }
        else
    ...
    static void IL_DaddyLongLegs_Act(ILContext il)
    {
        ...
        {
            c.Emit(Ldarg_0);
            c.EmitDelegate((float num, DaddyLongLegs self) => self.safariControlled && self.Template.type == PBEnums.CreatureTemplateType.LMiniLongLegs ? 0.45f : num);
        }
        else
    ...
    static void SLOracleBehaviorHasMark_CreatureJokeDialog(On.SLOracleBehaviorHasMark.orig_CreatureJokeDialog orig,  SLOracleBehaviorHasMark self) {
        orig(self);
        if (self.CheckStrayCreatureInRoom() == PBEnums.CreatureTemplateType.LMiniLongLegs) {
    ...
    static float OverseerAbstractAI_HowInterestingIsCreature(On.OverseerAbstractAI.orig_HowInterestingIsCreature orig, OverseerAbstractAI self, AbstractCreature testCrit) {
        if (testCrit?.creatureTemplate.type == PBEnums.CreatureTemplateType.LMiniLongLegs)
        {
            var num = .2f;
